...
export function verifyAccessToken(token: string): JwtPayload {
  try {
    ...
    if (decoded.type !== 'access') {
      throw new AuthenticationError(ErrorCode.TOKEN_INVALID, 'Invalid token type');
    }

    return decoded;
  } catch (error) {
    if (error instanceof jwt.TokenExpiredError) {
      throw new AuthenticationError(ErrorCode.TOKEN_EXPIRED, 'Token has expired');
    }
...
export function verifyRefreshToken(token: string): JwtPayload {
  try {
    ...
    if (error instanceof jwt.JsonWebTokenError) {
      throw new AuthenticationError(ErrorCode.TOKEN_INVALID, 'Invalid refresh token');
    }
    throw error;
  }
}
